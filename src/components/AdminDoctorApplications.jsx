    ...
    const fetchApplications = async () => {
        setLoading(true);
        console.log("Отправляю запрос на получение заявок...");
        try {
            const { data } = await axios.get("/doctorApplications", config);
            console.log("Данные заявок с сервера:", data);
            setApplications(data);
        } catch (error) {
            console.error("Ошибка в запросе заявок:", error);
    ...
    const handleSearch = async () => {
        ...
        const { data } = await axios.get(
            `//doctorApplications/search?specialization=${encodeURIComponent(search)}`,
            config
        );
        setApplications(data);
        } catch {
    ...
    const handleDelete = async (id) => {
        ...
        try {
        await toast.promise(axios.delete(`/doctorApplications/${id}`, config), {
    ...
    return (
        ...
        {loading ? (
            ...
            <thead>
                ...
                <th style={styles.th}>Контакт (телефон)</th>
